...
We will export the results of the t-test of whater or not the mean is
euqal to zero. When the mean is equal 0 we will fail to reject 5% of the
time, when we get a mean of 5 we will reject more times.

making two plots what the estimated mean is when the mean equal to 0.
Filter when the estimate p-values is 0.5 and simulate what it would look
like with whether it was significant or not.
